...
<ul class="pagination">
	<li><tags:url servlet="DashBoard" page="${(page-1<begin)?begin:page-1}" search="${search}" orderBy="${orderBy}" isDesc="${isDesc}">&laquo;</tags:url></li>
		<c:forEach var="nombre" begin="${(page-interval)<begin?begin:page-interval}" end="${(page+interval)>end?end:page+interval}">
			<li>
			
			<tags:url servlet="DashBoard" page="${nombre}" search="${search}" orderBy="${orderBy}" isDesc="${isDesc}">${nombre}</tags:url></li>
			
			<!-- <a href="?search=${search}&page=${nombre}&orderBy=&="></a></li>  -->
		</c:forEach>
  	<li><tags:url servlet="DashBoard" page="${(page+1>end)?end:page+1}" search="${search}" orderBy="${orderBy}" isDesc="${isDesc}">&raquo;</tags:url></li>
</ul>